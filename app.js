...
function npProfilesFromDetail(d,r){const entries=npEntriesFromDetail(d);const profiles=[];for(const np of entries){if(!np||typeof np!=='object')continue;const card=npCardCodeFromValue(np.card)||npCardCodeFromValue(np.cardType)||npCardCodeFromValue(np.commandType)||npCardCodeFromValue(d?.cardType)||'';const effects=npEffectTypesFromDetail(np);if(card&&effects.length){profiles.push({card,types:effects})}else if(effects.length){profiles.push({card:'',types:effects})}}if(!profiles.length){const card=npCardCode(r,d);const effects=npEffectTypesFromDetail(d);if(card&&effects.length)profiles.push({card,types:effects});else if(card)profiles.push({card,types:[]});else if(effects.length)profiles.push({card:'',types:effects})}const seen=new Set();return profiles.filter(p=>{const key=`${p.card}:${p.types.join(',')}`;if(seen.has(key))return false;seen.add(key);return true})}
function npProfileKeys(d,r){const profiles=npProfilesFromDetail(d,r);return profiles.flatMap(p=>p.types.map(t=>p.card?`${p.card}:${t}`:t))}
function npTypeText(d,r){const profiles=npProfilesFromDetail(d,r);return profiles.length?profiles.map(p=>p.types.length?p.types.join(' · '):'Type non défini').join(' · '):'—'}
function npProfilesHtml(d,r){const profiles=npProfilesFromDetail(d,r);if(!profiles.length)return '<span class="np-profile-empty">Type NP indisponible</span>';return `<div class="np-profile-list">${profiles.map(p=>`<div class="np-profile"><span class="np-profile-card">${p.card?cardImg(p.card):''}</span><span class="np-profile-types">${p.types.length?p.types.map(t=>`<b>${t}</b>`).join(''):''}</span></div>`).join('')}</div>`}
...
function openModalBase(r,d,urls,idx){
 ...
 const npProfiles=npProfilesHtml(d,r);const npHtml=can?`<div class="np-edit-wrap">${npIcon()}<div class="np-edit-main"><div class="np-value-line"><input class="editable-input np-input" id="e-np" type="number" value="${npValue}" min="1" max="999"><span class="np-display-hint">Affiché ${npRead}</span></div>${npProfiles}</div></div>`:`<div class="np-detail-row">${npIcon()}<div><b>${npRead}</b>${npProfiles}</div></div>`;
